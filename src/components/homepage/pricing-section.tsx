        ...
        <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {options.map((option, index) => {
            ...
            return (
              <Card 
                key={index} 
                className={`group relative rounded-2xl bg-card ${option.borderColor} ${
                  option.popular 
              ...
              >
                {option.popular && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                    <Badge className="text-primary-foreground border-0 rounded-full bg-primary">
                      Most Popular
                    </Badge>
                  </div>
                )}
                <CardHeader>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <CardTitle className="text-2xl text-foreground">{option.name}</CardTitle>
                  </div>
                  <CardDescription className="text-lg text-muted-foreground">{option.description}</CardDescription>
                </CardHeader>
